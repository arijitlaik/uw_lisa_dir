...
outputDirName = "tD_3d_2wlR8_DenLim_bNuLc_wLC_VutUc_thDc_eta0_1e3_Lc1e3_nlLM_2dot5e15_sLc"
...
vRes = 64
resMult = 2.0  # 64 being the base vRes
aRatioMesh = [2, 0.5]  # xRes/yRes,zRes/yRes
aRatioCoor = [4, 4]  # Model len ratio
yRes = int(vRes * resMult)
xRes = int(vRes * aRatioMesh[0] * resMult)
zRes = int(vRes * aRatioMesh[1] * resMult)
refineHoriz = True
refineVert = True
...
mesh = uw.mesh.FeMesh_Cartesian(
    ...
    minCoord=(nd(0.0 * u.kilometer), nd(-modelHeight), nd(0.0 * u.kilometer)),
    # maxCoord=(nd(9600.*u.kilometer), nd(192.*u.kilometer)),
    maxCoord=(aRatioCoor[0] * nd(modelHeight), nd(0.0 * u.kilometer), aRatioCoor[1] * nd(modelHeight)),
...
if restartFlag is False:
    velocityField.data[:] = [0.0, 0.0, 0.0]
...
freeslipBC = uw.conditions.DirichletCondition(
    variable=velocityField, indexSetsPerDof=(iWalls, jWalls, kWalls)
)
...
mantleShape = make_layer2d(
    startX=0.0,
    topY=0.0,
    length=nd(modelHeight * aRatioCoor[0]),
    thicknessArray=[nd(modelHeight)],
...
slabWidth = (fn.coord()[2] <= nd(2.*modelHeight)) & (fn.coord()[2] >= nd(0.2*modelHeight))
...
indentorWidth = (fn.coord()[2] <= nd(0.8*modelHeight)) & (fn.coord()[2] >= nd(0.2*modelHeight))
...
overRidingWidth = (fn.coord()[2] <= nd(2*modelHeight))
...
modelMaterials = [
    ...
    {
        "name": "Weak UL Crust Indian Indentor",
    ...
    {
        "name": "Weak LL Crust Indian Indentor",
...
#         file.close()
if uw.rank() == 0:
    print("Laying out materials.....")
if restartFlag is False:
    for i, mat in enumerate(modelMaterials):
        if mat.get("shape"):

            shapeLayout = fn.shape.Polygon(np.array(mat["shape"]))
            if mat.get("width"):
                shapeLayout = shapeLayout & mat["width"]
            condition = [
                (shapeLayout, i),
...
if uw.rank() == 0:
    print("Laying out materials..... Done!")

if uw.rank() == 0:
    print("Mapping Viscosity and Density Functions ..... ")
...
if uw.rank() == 0:
    print("Mapping Viscosity and Density Functions ..... Done!")

sf = dm(1.0, 1.0 * u.kilogram / u.meter ** 3)
if uw.rank() == 0:
    print("Checkpointing Initial Layout.....")
uw.barrier()
if restartFlag is False:
    checkpoint(
        None,
        None,
        swarm,
        {"materials": materialVariable},
        index=0,
        modeltime=dm(time, 1.0 * u.megayear).magnitude,
        prefix=outputDir,
    )
if uw.rank() == 0:
    print("Checkpointing Initial Layout: Swarm Done!")
if restartFlag is False:
    checkpoint(
        mesh,
        {
...
if uw.rank() == 0:
    print("Checkpointing Initial Layout: Mesh Done!")
# exit()
uw.barrier()
if uw.rank() == 0:
    print("Init.... Solver")
