...
outputDirName = "v27_TEST_opTe_2x12_512x256"
...
def checkpoint(step, time):
    ...
    materialVariable.xdmf(outputDir+'materialVariable.'+str(step).zfill(5)+'.xdmf',
                          materialVariableHnd, "materialVariable",
                          swarmHnd, "swarm", modeltime=time)
    velocityField.xdmf(outputDir+'velocityField.'+str(step).zfill(5)+'.xdmf',
                       velocityHnd, "velocity",
                       meshHnd, "mesh", modeltime=time)
...
surfaceArea = uw.utils.Integral(
    fn=1.0, mesh=mesh,
    integrationType='surface', surfaceIndexSet=top)
surfacePressureIntegral = uw.utils.Integral(
    fn=pressureField, mesh=mesh,
    integrationType='surface', surfaceIndexSet=top)
