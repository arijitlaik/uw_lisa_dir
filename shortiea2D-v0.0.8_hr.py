...
outputDirName = "Short_RefTest_3x16_8-00125_75"
...
if restartFlag is False:
    ...
    xG = np.geomspace(cenX + refRange[0]/2, mesh.maxCoord[0], mesh.elementRes[0]/2. - xL.size+1)
    assert mesh.elementRes[0]/2 - (xL.size + xG.size) == -1
    xR = np.concatenate((xL, xG), axis=0)
    xrF = np.flip((mesh.maxCoord[0]-mesh.minCoord[0])-xR)
    xR = np.concatenate((np.flip((mesh.maxCoord[0]-mesh.minCoord[0])-xR), xR), axis=0)
    ...
    yG = np.geomspace(mesh.maxCoord[1] + refRange[1], mesh.minCoord[1], yO.size - yL.size)
