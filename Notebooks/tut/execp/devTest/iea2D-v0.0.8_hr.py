...
outputDirName = "PerCellRandomLayout_ncfl"
...
if restartFlag is False:
    ...
    xL = np.arange(cenX, cenX + refRange[0]/2., refInt[0])
    xG = np.geomspace(
    ...
    xrF = np.flip((mesh.maxCoord[0]-mesh.minCoord[0])-xR)
    xR = np.concatenate(
        (np.flip((mesh.maxCoord[0]-mesh.minCoord[0])-xR), xR), axis=0)
    ...
    yL = np.arange(mesh.maxCoord[1],
                   mesh.maxCoord[1] + refRange[1], -refInt[1])
    yG = np.geomspace(mesh.maxCoord[1] + refRange[1],
                      mesh.minCoord[1], yO.size - yL.size)
    assert mesh.elementRes[1]+1 - (yL.size + yG.size) == 0
    yR = np.concatenate((yL, yG), axis=0)
    yR = np.flip(yR)  # -ve Coordinates silly hack for interp
...
swarmLayout = uw.swarm.layouts.PerCellRandomLayout(swarm=swarm,particlesPerCell=20)
